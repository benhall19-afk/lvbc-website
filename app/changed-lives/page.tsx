...
import { client, testimonyQuery } from '@/lib/sanity'

export const metadata: Metadata = {
  title: 'Changed Lives',
  description: 'Personal testimonies of lives changed by the gospel of Jesus Christ.',
}

async function getTestimonies() {
  if (!process.env.NEXT_PUBLIC_SANITY_PROJECT_ID) return []
  try {
    return await client.fetch(`*[_type == "testimony"] | order(publishedDate desc){ _id, title, slug, author, language, publishedDate }`)
  } catch {
    return []
  }
}
    ...
    <div className="min-h-screen" style={{ background: 'var(--bg)' }}>
      ...
      >
        ...
        <p className="text-lg opacity-90 max-w-2xl mx-auto px-4">
          Real people. Real stories. Lives transformed by the power of the gospel.
        </p>
      </div>
            ...
            {testimonies.map((t: any) => (
              ...
              >
                <div className="flex items-center gap-2 mb-3">
                  ...
                  {t.language === 'es' && (
                    <span className="ml-auto text-xs px-2 py-0.5 rounded-full" style={{ background: '#fef3c7', color: '#92400e' }}>ES</span>
                  )}
                </div>
